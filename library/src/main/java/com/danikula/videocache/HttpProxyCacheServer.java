...
import com.danikula.videocache.file.*;
...
public class HttpProxyCacheServer {
    ...
    //全部缓存
    public void startCache(final String url){
        stopCache(url);
        cachedThreadPool.submit(new HtttpCacheRunnable(url,0,-1));
    }

    //Cache部分 单位：byte
    public void startCache(final String url , long start , long end){
        stopCache(url);
        cachedThreadPool.submit(new HtttpCacheRunnable(url,start,end));
    }

    public boolean stopCache(String url){
        synchronized (clientsLock) {
            Source source = sourceMap.get(url);
            if(source != null){
                try
                {
                    source.close();
    ...
    public boolean stopCacheAndPlay(String url){
        synchronized (clientsLock) {
    ...
    private final class HtttpCacheRunnable implements Runnable {

        private final String url;
        private final long startByte;
        private final long endByte;

        public HtttpCacheRunnable(String url,long startByte,long endByte) {
        ...
        public void run() {
            ...
            Source source = sourceMap.get(url);
            if (source == null) {
                source = new HttpUrlSource(getProxyUrl(url));
                sourceMap.put(url, source);
            }
            try
            {
